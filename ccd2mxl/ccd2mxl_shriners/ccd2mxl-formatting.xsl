...
<xsl:stylesheet version="2.0" xmlns:xs="http://www.w3.org/2001/XMLSchema"
	...
	<xsl:template name="get-participant">
		...
		<p>
			<xsl:call-template name="get-name">
				<xsl:with-param name="name" select="$participant/associatedPerson/name" />
			</xsl:call-template>
			<xsl:if test="$participant/addr">
				<xsl:call-template name="get-address">
					<xsl:with-param name="addr" select="$participant/addr" />
				</xsl:call-template>
			</xsl:if>
			<xsl:if test="$participant/telecom">
				<xsl:call-template name="get-telecom">
					<xsl:with-param name="telecom" select="$participant/telecom" />
				</xsl:call-template>
			</xsl:if>
	...
	<xsl:template name="format-date-node">
		<xsl:param name="date-node" />
		<xsl:param name="short" required="no">1</xsl:param>

		<xsl:choose>
			<xsl:when test="$date-node/@value != ''">
                            <xsl:value-of select="$date-node/@value" />
			</xsl:when>
			<xsl:when test="$date-node/low/@value != '' or $date-node/low/@nullFlavor = 'UNK'">
                            <xsl:value-of select="$date-node/low/@value" />
			</xsl:when>
			<xsl:when test="$date-node/center/@value != ''">
                            <xsl:value-of select="$date-node/center/@value" />
			</xsl:when>
			<xsl:otherwise>
                            <xsl:value-of select="$date-node/high/@value" />
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
